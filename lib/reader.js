...
Reader.prototype.parseString = function parseString(input, options) {
	...
	// TODO: Pull schema from Mongoose then overload with inline docs
			...
			paths: _(blocks)
				...
				.mapValues(path => _(path)
					...
					.mapValues(b => {
						let response;
						if (b.returns) {
							const match = /^(?<wrapper>.*)\<(?<type>.+?)\>$/.exec(b.returns.type)?.groups;
							if (match?.wrapper === 'array') {
								response = {
									type: 'array',
									items: {
								...
								response = { $ref: '#/components/schemas/' + b.returns.type };
							}
						}
