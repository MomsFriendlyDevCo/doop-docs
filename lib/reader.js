			...
			paths: _(blocks)
				...
				.mapValues(path => _(path)
					...
					.mapValues(b => {
						let response;
						if (b.returns) {
							// FIXME: Make everything else in regex optional
							let match = /^(?<wrapper>.*)\<(?<type>.+?)\>$/.exec(b.returns.type)?.groups;
							if (!match) match = { type: b.returns.type };

							// FIXME: Object<{count: Number
							...
							match.type.split('|').forEach(type => {
								...
								if (type === 'Object') {
									...
									const match = /^\/api\/(?<collection>.+?)\//.exec(b.route.path)?.groups;
									if (match) type = match.collection;
								}
								// }}}

								// Preconfigured type and schema {{{
								if (type && _.has(options.responseTypes, type)) {
									switch (match?.wrapper) {
										case 'array':
										// TODO: Support for other types of "wrapper"?
										//case 'object':
											response.content[options.responseTypes[type].type] = {
												schema: {
								...
								// TODO: Fallback condition? Lowercase type?	
								//} else {
						...
						return {
							...
							...(b.params ? {
								parameters: b.params
									...
									// TODO: No place in OpenAPI schema for multipart-form body param descriptions?
									requestBody: {
										description: 'Form Data',
										required: (b.params.length > 0),
										content: {
											'multipart/form-data': {
												schema: {
													type: 'object',
													properties: _(b.params)
														.filter(param => _.isPlainObject(param) && Object.prototype.hasOwnProperty.call(param, 'name') && (
															param.name.startsWith('req.body.')
														))
														.mapValues(p => {
															// Extracting default from key="value" style names
															let match;
															if (match = /^(?<key>[\w\.]+)=(?<val>[^\s]+)$$/.exec(p.name)) {
																p.name = match.groups.key;
																p.default = match.groups.val
															}
															return p;
														})
