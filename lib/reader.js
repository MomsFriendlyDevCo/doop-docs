							...
							b.returns.type.split('|').forEach(type => {
								...
								if (_.has(options.responseTypes, match.type)) {
									switch (match?.wrapper) {
										// TODO: Will this wrapper ever be "object"?
										case 'array':
								...
								// Custom schema response {{{
									...
									switch (match?.wrapper) {
										...
										// FIXME: array<string> = items: { '$ref': '#/components/schemas/string' }
										case 'array':
											response.content['application/json'] = {
												schema: {
													title: pluralize.plural(match.type),
											...
											if (builtins.includes(match.type)) {
												response.content['application/json'].schema.items = { type: match.type };
											} else {
												response.content['application/json'].schema.items = { $ref: '#/components/schemas/' + match.type };
											}
											break;
										default:
											response.content['application/json'] = {
											...
											if (builtins.includes(match.type)) {
												response.content['application/json'].schema = { type: match.type };
											} else {
												response.content['application/json'].schema = { $ref: '#/components/schemas/' + match.type };
											}
											break;
									}
								}
								...
								parameters: b.params
									...
									.map(param => {
										...
										if (match = /^(?<key>[\w\.]+)=(?<val>[^\s]+)$$/.exec(param.name)) {
											...
											// FIXME: What about function/true/false/null/undefined which have no quotes around them?
