		...
		context: (block, lines) => { // Determine context from next code lines - return truthy when found or false if nothing
			...
			return lines
				.find(line => {
					...
					// TODO: Include default methods from `app.use('/api/orders/:id?', app.middleware.express.rest`?
					...
					// TODO: Single line processing, can't see whole schema.
					//} else if (match = /^app\.middleware\.db\.schema\((?<speachmark>['"`])(?<collection>.+?)\k<speachmark>\s*,\s*(?<schema>\{.*?\})\)/.exec(line)?.groups) { // Looks like a doop schema (e.g. `app.middleware.db.schema('collection', {})`)
						//debug('SCHEMA', JSON.stringify(JSON.parse(match.schema), null, 2));
					//	debug('SCHEMA', match);
					//	console.warn('[WARN]', 'TODO: Translate schema into response');
