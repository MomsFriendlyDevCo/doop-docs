				...
				.then(contents => {
					...
					for (collection in contents.oapi.components.schemas) {
						...
						_.merge(contents.oapi.components.schemas[collection], {
							properties: _(app.db[collection]?.schema?.paths || app.db[collection]?.$mongooseModel?.schema?.paths) // Support either Mongoosy or Monoxide
								.mapValues(p => {
									var t = {};
									switch (p.instance) {
										case 'Date':
					...
					for (path in contents.oapi.paths) {
						...
						debug('controller', controllerName, controllerNameSingular);

						for (method in contents.oapi.paths[path]) {
							debug('path', path);
							debug('method', method);
							debug('params', match.groups?.params);

							// Define unique operation identity {{{
							let operationId;
							switch (match.groups?.type) {
								case 'api':
									// Categorise paths by controller, leaving them if tags already exist (from optional `@category` usage)
									if (contents.oapi.paths[path][method].tags && contents.oapi.paths[path][method].tags.length > 0) {
										console.log(`[INFO] Method ${method} on path ${path} already has tags (${contents.oapi.paths[path][method].tags}), not adding tag from controller name`);
										continue; // TODO: Rather than skipping; Should we set "controllerName" and "controllerNameSingular" to what is given by the @category?
									}
							...
							if (!contents.oapi.paths[path][method].operationId && operationId)
								contents.oapi.paths[path][method].operationId = operationId;

							debug('operationId', contents.oapi.paths[path][method].operationId);
							debug('tags', contents.oapi.paths[path][method].tags);
							// }}}
					...
					// Extract all used security policies {{{
					if (!_.has(contents.oapi.components, 'securitySchemes')) {
						contents.oapi.components.securitySchemes = {};
					}
					for (let pathName in contents.oapi.paths) {
						const path = contents.oapi.paths[pathName];

						for (let method in path) {
							const operation = path[method];
							if (operation.security) {
								for (let security of operation.security) {
									for (let schemeName in security) {
										if (!_.has(contents.oapi.components.securitySchemes, schemeName)) {
											console.log(`Adding top level security scheme: '${schemeName}', because it is first used in path: ${pathName}`)
											contents.oapi.components.securitySchemes[schemeName] = {
												type: 'apiKey',
												in: schemeName == 'apiKey' ? 'header' : schemeName == 'cookie' ? 'cookie' : 'query',
												// description: 'Custom security scheme used for paths: ' + pathName,
												name: schemeName,
											};
										} else {
											// TODO: Anything desired here?
											// contents.oapi.components.securitySchemes[schemeName].description += ', ' + pathName;
										}
									}
								}
							}
