						...
						for (method in contents.oapi.paths[path]) {
							...
							switch (match.groups?.type) {
								case 'api':
									...
									const obj = contents.oapi.paths[path][method];
									if (obj.tags && obj.tags.length > 0) {
										console.log(`[INFO] Method ${method} on path ${path} already has tags (${obj.tags}), not adding tag from controller name`)
										continue
									}
									obj.tags = [controllerName];
							...
							if (operation.security) {
								for (let security of operation.security) {
									for (let schemeName in security) {
										const scopes = security[schemeName];
